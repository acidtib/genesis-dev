...
{
    // return if URI is not valid or is no weycash: URI
...
{
    // Convert weycash:// to weycash:
    //
    //    Cannot handle this later, because weycash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open weycash.conf with the associated application */
...
{
    return GetAutostartDir() / "weycash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a weycash.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the weycash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add weycash app to startup item list
